...
# heapq 모듈: 이진트리 기반의 "최소 힙" 자료구조를 제공한다.
# Java에서 PriorityQueue 클래스와 유사.

# 빈 리스트를 생성후 heapq 모듈 함수 호출때마다 이 리스트를 인자로 넘김.
# heapq 모듈을 통해 원소를 추가하거나 삭제한 리스트가 최소 힙이 된다.
# 힙의 원소를 추가할 때는 heappush(리스트, 원소) 함수를 이용.
# 힙의 원소를 제거할 때는 heappop(리스트). 가장 작은 원소를 삭제한다.
# 기존 리스트를 힙으로 만들고싶으면 heapify(기존리스트)를 사용한다.
